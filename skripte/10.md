...
Mogu da se prosleđuju podaci koji se nalaze izvan testova pomoguću 
konfiguracionih fajlova. Preko njih možemo i da definišemo koji će 
testovi da se izvršavaju, a koji ne.

Ono što je još dobro je to što možemo da generišemo specijalizovane izveštaje.
...
Ono što je bitno imati u vidu jeste to da je test širi pojam u TestNG-u u odnosu na JUnit.
...
Postoji više tipova lokatora, tj. strategija lociranja:
* ID elementa
* Name elementa
* CSS selektori
* XPath
...
Submit nas vodi na novu stranicu pa je poželjno utvrditi da li je došlo do prelaska
na drugu stranicu.
...
Pri validaciji rezultata **poželjno je da se proveri da li nađeni element nije null**.

---

Ukoliko sada želimo da testiramo stranicu koja predstavlja rezultat pretrage,
ne moramo ponovo vršimo unos, već **možemo da iskoristimo adresu na koju smo 
prosleđeni**, tj. `https://www.tehnomedia.rs/search/find.html?rec=monitor`, i
početi rad od te andrese. U ovom primeru to možemo da odradimo jer URL sadrži 
query deo koji nam daje dati rezultat, ali to **ne možemo uvek raditi**.
